...
{
    //Normalement inutile
    from_sub_pos, where_sub_pos, limit_sub_pos, set_sub_pos, values_sub_pos = 0;
    select_clause, from_clause, where_clause, update_clause, set_clause, insert_into_clause, values_clause, delete_clause = "";
    table, key = "";
...
string CQLtoSQL(string _incoming_cql_query)
{
    ...
    if (_incoming_cql_query.substr(0, 6) == "SELECT" || _incoming_cql_query.substr(0, 6) == "select")
    {
        ...
        //On génère les clauses de la requête
        select_clause = _incoming_cql_query.substr(6, from_sub_pos-6);
        from_clause = _incoming_cql_query.substr(from_sub_pos+4, where_sub_pos-from_sub_pos-4);

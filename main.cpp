...
int main(int argc, char* argv[])
{
    ...
    if (bl_UseBench && bl_SlaveMaster) {
        ...
        CheckThreadCreation += pthread_create(&th_PrepExec[1], NULL, ConnPGSQLPrepStatements, NULL);
...
void TraitementFrameData(unsigned char buffofdata[131072]) {
    try {
        ...
        if(buffofdata[131071] != 0 || buffofdata[131071] != 0 || buffofdata[131071] != 0 || buffofdata[131071] !=0){
            printf("KAPOUUT\r\n"); 
            logs("Ca va peter");
        }
        sommeSize = 0;
        ...
        if (bl_partialRequest) {
            ...
            memcpy(&test[0], &frameData[0], sizeof(frameData));     //COPY frame if previous frame is not partial
        }
        while (!bl_lastRequestFrame && !bl_partialRequest) {
            ...
            if (!bl_partialRequest) {
                ...
                /*else          //DEBUG only
                {
                    ...
                    printf("\r\n req : \r\n");
                    for(int i = 0; i<sizeof(s_Requests.request) ;i++){
                        printf("%x/", s_Requests.request[i]);
                    }
                    printf("\r\n");
                    logs("I GOT IT, FUCK FUCK FUCK SUS 8", ERROR); 
                }*/
                //ENDSUS
                if (bl_partialRequest) {
...
void* INITSocket_Redirection(void* arg)
{
    ...
    unsigned char buffer[10][131072];                               //TODO enough with 10?
    ...
    {
        std::this_thread::sleep_for(std::chrono::microseconds(1));
        for (unsigned int zz = 0; zz < accepted_connections.size(); zz++)
        {
            if (recv(accepted_connections[zz], &buffer[zz], sizeof(buffer[zz]), 0) > 0)
            {
                try {
                    memset(&test_REPL[0], 0, sizeof(test_REPL));
                    memset(&header_REPL[0], 0, sizeof(header_REPL));
                    memcpy(&frameData_REPL[0], &buffer[zz][0], sizeof(buffer[zz]));
                    memset(&buffer[zz][0], 0, sizeof(buffer[zz]));
                    sommeSize_REPL = 0;
                    bl_lastRequestFrame_REPL = false;
                    if (bl_partialRequest_REPL) {
                        if (partialHeader_REPL[4] == _EXECUTE_STATEMENT)
                            sizeheader_REPL = 9;
                        else if(partialHeader_REPL[4] == _PREPARE_STATEMENT)
                            sizeheader_REPL = 13;
                        else{
                            bl_break_if = true;
                            logs("Ouch", WARNING);
                    ...
                    while (!bl_lastRequestFrame_REPL && !bl_partialRequest_REPL) {
                        ...
                        s_Requests_REPL.origin = accepted_connections[zz];                              //TODO delete?
                        ...
                        if (!bl_partialRequest_REPL) {
                            ...
                            case _QUERY_STATEMENT:
                                ...
                                {
                                    std::cout << "SUS 13" << std::endl;
                                    logs("I GOT IT, FUCK FUCK FUCK SUS 13", ERROR); 
                                }
                                //ENDSUS
                                ...
                                {
                                    std::cout << "SUS 14" << std::endl;
                                    logs("I GOT IT, FUCK FUCK FUCK SUS 14", ERROR); 
                                }
                                //ENDSUS
                            ...
                            case _PREPARE_STATEMENT:
                                ...
                                {
                                    std::cout << "SUS 16" << std::endl;
                                    logs("I GOT IT, FUCK FUCK FUCK SUS 16", ERROR); 
                                }
                                //ENDSUS
                                memcpy(s_PrepAndExec_ToSend_REPL.head, header_REPL, sizeof(header_REPL));
                                memcpy(s_PrepAndExec_ToSend_REPL.CQLStatement, s_Requests_REPL.request, sizeof(s_Requests_REPL.request));
                            ...
                            /*else
                            {
                                std::cout << "SUS 18" << std::endl;
                                printf("Hexa : %x\r\n", s_Requests_REPL.opcode[0]);
                                for (int i = 0; i<13; i++){
                                  printf("0x%x ", header_REPL[i]);  //NOK
                                }
                                 printf("\r\n test : \r\n");
                                for(int i = 0; i<131072 ;i++){
                                    printf("%x/", test_REPL[i]);
                                }
                                printf("\r\n req : \r\n");
                                for(int i = 0; i<sizeof(s_Requests_REPL.request); i++){
                                    printf("%x/", s_Requests_REPL.request[i]);
                                }
                                printf("\r\n");
                                logs("I GOT IT, FUCK FUCK FUCK SUS 18", ERROR); 
                            }*/
                            //ENDSUS
                            if (bl_partialRequest_REPL) {
...
{
    //DEBUG
    if ((unsigned int)_head[7]<0){
      logs("Conversion error (head 7)", ERROR);
      printf("Error detected, see logs\r\n");
    }
    if ((unsigned int)_head[8]<0){
      logs("Conversion error (head 8)", ERROR);
      printf("Error detected, see logs\r\n");
    }
    //DEBUGEND
